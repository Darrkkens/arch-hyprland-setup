...
lock_file="$cache_dir/media-cover.lock"
...
cover_url="$(timeout 1s playerctl --player="$player" metadata mpris:artUrl 2>/dev/null || true)"
...
if [[ "$cover_url" != "$current_url" || ! -s "$cover_file" ]]; then
	...
	if curl --connect-timeout 2 --max-time 4 --fail --silent --show-error --location "$cover_url" --output "$temp_file"; then
